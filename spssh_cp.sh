...
function usage() {
    echo 1>&2 "Usage: spssh_cp.sh [--find-args '-maxdepth 1 -name \*.sh ..']"
    echo 1>&2 "                   [--compress-program none/gzip/zstd/..] [--safe-mode]"
...
sleep 0.5
echo " echo Receiving '\"$FILE\" ...'; mkdir -p '$DSTDIR'; bash -c \"stty -echo -icanon intr undef; $RECEIVE_CMD | base64 -d 2> /dev/null | dd bs=64K iflag=fullblock status=progress | tar x $COMPRESS_ARGS -C '$DSTDIR' 2> /dev/null || (echo 1>&2 -e '\nInterrupted by user'; sleep 3; exit 1)\" || exit 1"
(cd "$SRCDIR"; eval "$SENTINEL_BEFORE_CMD"; eval find "'$FILE'" "$FIND_ARGS" -print0 | tar cv $COMPRESS_ARGS --null -T - | dd bs=64K | base64 -w 4095; eval "$SENTINEL_CMD"; echo " stty echo icanon intr ^C; PS1=\$PSBAK; unset PSBAK")
